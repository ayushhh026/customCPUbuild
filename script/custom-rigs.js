...
console.log("custom-rigs.js is loaded and running"); // Debugging
...
function checkAllSelected() {
  ...
  dropdowns.forEach((dropdown) => {
    if (dropdown.selectedIndex === 0) { // Check if the default option is selected
      allSelected = false;
    }
  });
  ...
  if (allSelected) {
    placeOrderButton.removeAttribute('hidden'); // Show the button
  } else {
    placeOrderButton.setAttribute('hidden', true); // Hide the button
  }
}
